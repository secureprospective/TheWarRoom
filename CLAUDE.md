...
**Next branch:** Continue on **`session/b2b-fetch-offense`** (NOT merged; crosswalk + nflproduction + extcsv seam + TouchShare + AgeTrajectory + RAS built/reviewed/live-verified, 8 commits ahead of main). **Open the next session by pasting `docs/build-handoffs/handoffs/06c-B2b-Fetch-Offense-veteranfilm.md`** (Veteran-Film + the pbp streaming seam change, then CFBD pair + Madden). The prior `06b` handoff is now mostly executed (3 of its 4 fetchers done); `06c` carries the remainder.
...
| Veteran-Film (FTN→PBP) + pbp streaming | **OPEN — NEXT, the hard one.** Needs an extcsv streaming variant first (pbp is hundreds of MB; 64 MiB cap fails by design). Emit RAW per-player trait RATES (zero-leak "how" not "how much"). Handoff `06c` |
