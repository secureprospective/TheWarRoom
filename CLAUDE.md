...
| CollegeProductionShare (CFBD) | **GATED — NEXT.** Blocked on the **rookie player-id keying** decision: CFBD `playerId` → MFL/gsis for college-only players the crosswalk omits (Christopher's call + recon: CFBD `/draft/picks`? name+college match? defer to rookie-import?). Long-format `/stats/player/season`, share = Σplayer÷Σteam, no targets→reception/yardage share. Handoff `06d` |
| Madden (EA ratings-api) | **OPEN — after CollegeShare.** Current-season EA API BLOCKED (m26 500s); historical slug works for birthdate. Lower-durability fallback. JSON, no-auth, no extcsv |
