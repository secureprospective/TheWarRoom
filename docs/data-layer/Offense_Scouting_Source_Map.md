...
- **CFBD API key (Christopher, ~2 min):** `collegefootballdata.com/key`, enter email, paste key back. Endpoint 401s without it; none stored on box. THEN: run a live `/stats/player/season?category=receiving` call to confirm whether CFBD exposes **targets**. Strong prior: **college box scores don't track targets** — so receiver `CollegeProductionShare` will likely be **reception/yardage share, not target share**. Confirm with the keyed call. (Gemini claimed 240 req/min; unverified.)
