...
Status: **RESOLVED 2026-07-03 → `int64` cents (scaled-integer, exactness-by-construction).** Decided at the B7a gate via an independent two-expert panel (GLM 5.2 + Gemini, self-contained parallel brief), triaged against each other + source. **Both ranked A (int64 minor units) > C (decimal/rational) > B (float64+discipline), HIGH confidence.** Deciding factor: dead-cap is fractional + accumulated + summed across players/years, and cap flows into the output-store's exact-equality tiebreak via L5 cap scaling — the exact silent, insertion-order-dependent failure mode float64 manufactures. **Unit = cents (not whole dollars):** pro-rata dead-cap (per-game ÷17/18, restructure ÷5) yields sub-dollar fractions; cents costs nothing (league total ~8e11 cents vs int64 9.2e18 ceiling, and < 2⁵³ so Wails IPC-safe). Locked mechanics: (1) **largest-remainder (Hamilton) distribution** — round the total ONCE at cents, then integer-distribute the remainder, deterministic tie-break earliest-year-first, so `Σ splits == total` exactly; (2) **additive migration** — `ADD COLUMN *_cents INTEGER`, backfill `CAST(ROUND(x*1e6) AS INTEGER)`, verify 0 mismatches, keep legacy one release then drop, `CHECK(*_cents >= 0)`; (3) **parse the MFL decimal STRING straight to int64 cents** — kill the `normalize.parseSalary` float64 intermediate (`internal/normalize/roster.go:103`), never round-trip money through float64 or through a JS number (string in, format at the render edge). **Companion league-rules calls (Christopher, same gate):** dead-cap v1 = SIMPLE (salary + signing-bonus proration only; per-game/incentive math deferred); **NO cap rollover** (so `cap_used ≥ 0` / `dead-cap ≥ 0` is a hard invariant). Test floor: conservation, split-conservation, round-trip idempotency, non-negativity, and the load-bearing **randomized-ordering sum-must-be-byte-identical** property test. This gates the B7a contract-ops/dead-cap TAIL only — the trade + roster-status v1 spine is money-free. Panel briefs + full answers archived in the B7a session. Noted OPEN 2026-06-18 (B3); resolved 2026-07-03 (B7a gate).
