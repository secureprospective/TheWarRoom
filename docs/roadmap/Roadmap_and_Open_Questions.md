...
The league's weekly matchup schedule — which of the 32 franchises play each other each week — is **commissioner-defined when the league goes live**, NOT derived from MFL's `nflSchedule` endpoint (that endpoint returns real NFL game matchups, ingested by `internal/ingestion/schedule` at B2, for live-scoring windows only — a separate concern, do not conflate). Christopher supplied the **schedule methodology** (2026-06-19) — a deterministic NFL-mirrored generator: 32 franchises (2 conf × 4 div × 4 teams ranked 1–4 by prior-year finish), 13-week regular season (6 division / 4 intra-conf inter-divisional / 3 cross-conference, only weeks 1 & 7 placement-driven) + weeks 14–17 playoffs (7 per conference, #1 byes, division winners seeded 1–4, wildcards by standard tiebreakers). Full spec at **docs/data-layer/League_Schedule_Methodology.md**.
Status: **RESOLVED (methodology)** 2026-06-19 — schema + generation rules locked. Generator build is a separate session. Remaining sub-items folded into the doc's Open Items: home/away designation, inaugural-season rank seeding, and whether the populated schedule is also readable back from MFL's fantasy `schedule` TYPE (live ingestion, distinct from B2's `nflSchedule`).
...
Status: **RESOLVED 2026-07-03 → `int64` cents (scaled-integer, exactness-by-construction).** Decided at the B7a gate via an independent two-expert panel (GLM 5.2 + Gemini, self-contained parallel brief), triaged against each other + source. **Both ranked A (int64 minor units) > C (decimal/rational) > B (float64+discipline), HIGH confidence.** Deciding factor: money must never be float, and cap flows into the output-store's exact-equality tiebreak via L5 cap scaling — a domain where float equality is unsafe. **CORRECTION (2026-07-04, rulebook audit):** the panel's original supporting rationale — "dead-cap is fractional + accumulated + insertion-order-dependent," "per-game ÷17/18, restructure ÷5" — was NFL-model leakage; NONE of it exists in the Official Rulebook. Every money operation in this league is FLAT MATH on $10k-rounded salaries (dead cap 35%×salary×years §8, restructure 50% §11, buyout 60/75/90% §12, extension 150%×highest year §10, tag = top-5-at-position average §9), so every intermediate is EXACT in integer cents by construction — no distribution, no rounding policy, no ordering sensitivity. The int64-cents CONCLUSION stands on its own merits (integer money + an exact cap tiebreak); the remainder-distribution scaffolding it was justified by is retired. **Unit = cents:** headroom + uniformity + JS/Wails IPC-safety (league total ~8e11 cents vs int64 9.2e18 ceiling, < 2⁵³). Locked mechanics: (1) **flat-formula math, exact in integer cents** — each obligation is a single flat-percentage product per the rulebook; there is NO fractional split across years/players, therefore no Hamilton/largest-remainder step and no insertion-order sensitivity (this replaces the retired distribution mechanic); (2) **additive migration** — `ADD COLUMN *_cents INTEGER`, backfill `CAST(ROUND(x*1e6) AS INTEGER)`, verify 0 mismatches, keep legacy one release then drop, `CHECK(*_cents >= 0)`; (3) **parse the MFL decimal STRING straight to int64 cents** — kill the `normalize.parseSalary` float64 intermediate (`internal/normalize/roster.go:103`), never round-trip money through float64 or through a JS number (string in, format at the render edge). **Companion league-rules calls (rulebook-grounded, corrected 2026-07-04):** dead-cap = the §8 waiver formula **35% × annual salary × remaining years** (50% if the contract was restructured, §11); ZERO if the player is claimed off waivers. There is **NO signing-bonus concept** in this league — that clause was NFL-model leakage and is STRUCK. **NO cap rollover** — the rulebook fixes a $125M/year cap with no carry-forward provision (§1), so `cap_used ≥ 0` / `dead-cap ≥ 0` is a hard invariant. Test floor: string→cents parse exactness, migration 0-mismatch (REAL↔cents backfill), round-trip idempotency, non-negativity (`CHECK(*_cents >= 0)`), and each contract-op formula pinned to its rulebook value. (The former split-conservation + randomized-ordering property tests are RETIRED — they exercised a fractional-distribution step this league does not have.) This gates the B7a contract-ops/dead-cap TAIL only — the trade + roster-status v1 spine is money-free. Panel briefs + full answers archived in the B7a session. Noted OPEN 2026-06-18 (B3); resolved 2026-07-03 (B7a gate).

**OQ-015: Automated nflverse Signals as Film/RAS Replacements**
B2b-Fetch-Offense source-access recon (2026-06-19, Claude + Gemini 3.1 cross-check, all endpoints verified live) surfaced free, Go-reachable nflverse datasets that could AUTOMATE inputs currently slated for manual entry: **combine.csv** (homebrew RAS via positional z-scores — a RAS-*equivalent*, not Kent Lee Platte's exact figure), **ftn_charting_{year}.csv** (broken tackles, routes run, air yards — a charting proxy for PFF/RSP-style film), and **ngs_{passing,receiving,rushing}** (separation/cushion; the receiving file is also the literal CB/S Coverage anchor for B2b-Fetch-Defense). Adopting any of these as a *scouting signal* is NOT a fetcher decision — it changes what a LOCKED rubric field (`PFFGrade`, `RSPQualitative`, `RAS`) measures and what it's calibrated against, so it requires a deliberate rubric revisit + recalibration, not a silent swap. Christopher's call (2026-06-19): **build B2b-Fetch-Offense against the locked rubric as-is** (manual manifest for PFF/RSP/DraftNetwork/Sharp/RAS); evaluate these replacements separately here. Zero-leak caution: each candidate field needs the field-by-field check (air yards / routes run are opportunity-ish — verify none encode fantasy points or format-dependent volume). **Reject FantasyPros `ecrData` outright** — it is Expert Consensus *fantasy* Rankings (fantasy-point-driven) → a zero-leak violation, and redundant with nflverse snap_counts for TouchShare.
Status: **RESOLVED (architecture) 2026-06-19 → Option D (Pragmatic Scouting Hybrid).** Full decision + verified source map at **docs/data-layer/Offense_Scouting_Source_Map.md**. Net: ELIMINATE manual entry — veterans 0 manual (FTN-charting film + nflverse/CFBD/EA), rookies 1 manual input/yr (a consensus draft-rank CSV, zero-leak scouting consensus). PFF/RSP/DraftNetwork/Sharp eliminated. This FORCES a Film-component redesign (for QB/WR/TE the eliminated sources were 100% of Film): veteran film = FTN composite (primary, join verified) + Madden (fallback — current-season EA API is BLOCKED/500, birthdates still safe via historical slug). Two locked principles from the dual vote: **durability never influences a rubric weight** (engineering concern → cache/fallback); **quality informs weights only as a fidelity discount via calibration**, not a "rank by source confidence" scheme. **The new film weights are UNSET — a calibration-against-data job (new CAL pass), not a blind number.** Remaining opens: CFBD key (mint + confirm targets→reception-share), Madden-current durability, college-birthdate coverage chain. Reject: FantasyPros ecrData (fantasy leak), CFBD PPA (points leak).
...
|---|---|---|---|
| OQ- | 016 | OQ-015 | Automated nflverse signals as film/RAS replacements |
