...
**Status:** Locked — **PARTIALLY SUPERSEDED by DECISION-011 (2026-06-19).** K Layer 4 is no longer a flat structural exclusion: K's Film component is now ACTIVE with sub-signals **Madden 0.60 / NFLProduction 0.40** (Madden is no longer archival-only). RAS and Breakout remain excluded (SL-020 / not-translatable). The mechanics rewrite of this rubric — Film cap/curve calibration + AD-10 and SL-020 reconciliation — executes at **B5b-K**; the sections below still describe the pre-DECISION-011 exclusion model until then. See docs/scoring-engine/Scouting_Schema.md → DECISION-011.
