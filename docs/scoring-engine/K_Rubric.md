...
**Status:** **SUPERSEDED by DECISION-011 — IMPLEMENTED at B5b-K (2026-06-28).** K Layer 4 is no longer a flat structural exclusion: K's Film component is ACTIVE, built as a **Madden 0.60 / NFLProduction 0.40** composite passed through the film S-curve at the pinned **±3% cap / steepness 10.0 / inflection 0.50** (Christopher's B5b-K gate-check; K is sparse-data → conservative cap). RAS is forced to exactly 1.000 (SL-020 partial, routed to L6) and Breakout to exactly 1.000 (no breakout framework at K), so `Combined = film`. **AD-10 is reversed and shipped** (film now moves K's valuation). The authoritative spec is `internal/engine/l4/kicker/k.go` + Scouting_Schema.md → DECISION-011. **⚠️ §2/§3/§4 BELOW ARE STALE** — they still describe the pre-DECISION-011 exclusion model (film_cap 0.00, "hardcoded 1.000", "Madden archival only"). DO NOT code from them; they remain only as historical record pending a full body rewrite.
**(Historical) Structural exclusion model** per Christopher's Option 1 call — Layer 4 scouting layer output hardcoded to flat 1.000 multiplier across all three components. True ranking at K is driven by Layer 2 raw production, Layer 3 age depreciation, and Layer 5 cap efficiency. This rubric is the cleanest application of SL-020 in the architecture: RAS-not-applicable, film-not-meaningfully-scoutable, breakout-framework-doesn't-translate. Gemini's K open questions reconciled into SL-OQ-042; Layer 6 scarcity-sort preservation confirmed by Gemini's verification check.
