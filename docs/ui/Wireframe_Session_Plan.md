...
- **SESSION C (Color, Dark Mode & Atmosphere) CONFIRMED 2026-07-19 — "perfect pass" (Christopher, vision gate).** 3 GLM-5.2 runs (tactical-ops warm / naval-CIC cold divergent + synthesis), triaged leads-not-findings; deliverable = the token sheet on real A+B surfaces + a one-page atmosphere board at `docs/ui/wireframes/session-c/session-c-wireframe.html` (branch `session/ui-session-a-grid`). **Confirmed decisions now bind Sessions D–E and the whole build track:** **base = cold CIC** desaturated navy, `--surface-*` ramp on `hsl(220,…)` (canvas 20%/6% → sunken 4% → tile 9% → raised 11% → overlay 13%), elevation optical-only (bevel-hi/lo + hairline, no dropshadow); **semantic ramps** muted/base/loud tuned for the cold base (green `145,45%,55%` / blue `210,60%,62%` / amber `35,60%,58%` / red `0,55%,62%` at base — Run-2 phosphor `#00FF9D` REJECTED as too loud); **score→hue banding** on the Adjusted-Score value + inspector layer bar ONLY (≥90 green-loud / 80-89 green-base / 70-79 blue-base / 55-69 amber-base / <55 red-base); **restraint doctrine** "color is Data + State, structure is achromatic" — no row/tile/canvas EVER tinted by value; **Matrix yields** all semantic hue to `--text-primary` except red-danger; **keyboard focus = blue** inset (never amber, a semantic); **selection = neutral axis** `--text-primary` (never green, selecting isn't a verdict); **delta-in-weight (Session B) is the baseline signal, hue is redundant reinforcement**; **freshness grammar composed** (`--fresh-*`: live=1px blue edge / cache=1px amber edge / failed=2px red edge + timestamp recolor + `(cache)`/`(offline)` suffix) under the guardrail that the DATA stays fully colored/legible in all three states; **motion** transforms/opacity only, ≤150ms, feedback-only, `--motion-mult:0` kills it in Matrix (4 named cases: feed-arrival 100ms / hold-to-fire 150ms blue→green fill / engine-reject red opacity flash / clock-<1hr amber). **Position badges = categorical, held OFF the 5 semantic hues:** QB violet 290 / RB magenta-rose 330 / WR teal 175 / TE indigo 250 / DL slate 220 / K achromatic — RB↔WR deliberately on opposite temperatures (155° apart) after Gemini flagged a real RB/WR blend; Gemini's neon (Option 1) + solid-pastel (Option 2) fixes REJECTED (re-collided with signal hues + broke restraint). **Command Ledger extended C1–C2 (freshness read-state + event escalation) plus the standing token contract.** **Session C atmosphere/tokens are a standing attachment to Sessions D–E.**
- **SESSION B (Component Hierarchy & Typography) CONFIRMED 2026-07-19 — "this is amazing, pass" (Christopher, vision gate).** 3 GLM-5.2 runs (aerospace-placard / terminal-forward divergent + synthesis), triaged leads-not-findings; fused component sheet at `docs/ui/wireframes/session-b/session-b-wireframe.html` (branch `session/ui-session-a-grid`, commit `c5fd157`). **Confirmed decisions now bind every later brief (C–E):** type — Inter=text / JetBrains Mono=data, hero numeric 24px/700 tabular right-aligned gray-50, section labels 11px upper 0.05em; **delta-in-weight** (positive Δ=weight 600 / negative=400, a greyscale-honest signal — Session C evaluates hue reinforcement but the weight encoding SHIPS); **typographic sort indicator** (▼/▲ active gray-50, ¦ inactive gray-800, no icon chrome); table rows have **no vertical gridlines** (optical spacing + row-hover bleed) inside the Session-A inset-bevel shell; 4 row states rest/hover(raised inset)/selected(2px left axis)/active(pressed inset); M1 columns = the **locked 7-col facet map** Rank·Player·Pos·Franchise·Base·Adjusted·Salary (Matrix drops Franchise+Base, Narrative expands Player to 2 lines); **ConfirmModal = 480px CENTERED + HOLD-TO-FIRE** (single verb `txn.commit`, ≤600ms fill, release-before-complete cancels, engine-reject holds non-dismissable — the two-button ARM→FIRE was rejected); Inspector = score-dominant header + 6 layer bars (4px, gray-800 track/gray-100 fill) + terminal-style contract block; empty/loading/error = ONE class (engraved fresh-install / data-shaped skeletons not spinners / cache-only state-line+2px edge / offseason clean-no-edge); keyboard map J/K/Enter/T + 1/2/3 density + `/` + Esc, each a Ledger verb. **Command Ledger extended B1–B9.** **Session B is a standing attachment to Sessions C–E** (the confirmed component sheet is attached to the C brief).
