# SESSION A — Grid & Spatial System · RUN 2
...
**"DENSITY GRADIENT"** — one continuous grid where information PRESSURE, not layout slots, creates structure. There are no fixed zones; the workspace is a single field whose local density rises where the data demands attention and relaxes where it doesn't. Hierarchy emerges from how tightly packed and how loud a region is, not from walls between panels. Structure is discovered by reading the gradient, not by counting cells.
