# SESSION C — Color, Dark Mode & Atmosphere · RUN 2
...
**"NAVAL CIC — COLD MIDNIGHT BASE, PHOSPHOR-COOL SIGNALS"** — the atmosphere of a ship's Combat Information Center: a cold, deep midnight-blue base (dark ≠ gray, and here the temperature is decisively COOL — a deep desaturated navy anthracite, never warm), lit by phosphor-cool signal colors — the green of a radar return, the cyan-blue of a friendly track. The ground recedes into cold depth so the signals float above it like illuminated tracks on a plot. Against this cold base, AMBER and RED read as HEAT — warning and danger are the only warm things on the screen, so they command attention by temperature alone before the eye even reads the color. Push the CIC feel hard: think radar phosphor, backlit plot tables, the disciplined calm of a darkened combat bridge. Then be honest in RIPCORD about where the cool navy base muddies blue-info legibility (blue signal on blue-ish ground), or where the cold ground makes the resting console feel clinical rather than commanding.
