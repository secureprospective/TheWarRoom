# SESSION E — Mobile Learning Harvest (chat-first doctrine) · RUN 1
...
**"THE INBOX — THE PHONE IS A REACTIVE TRIAGE STATION."** The organizing truth of mobile is that a phone is where things REACH you — you do not sit at a phone to run a portfolio, the phone taps you on the shoulder. So design mobile as an ALERT-TRIAGE instrument first: the primary surface is the escalation/ALERTS-tributary grammar from Session D, promoted to the whole shell. The phone's job is "here is the one thing that needs you NOW, and here is the single gesture that resolves it." Push this hard: most desktop surfaces DIE or become PUSH NOTIFICATIONS on mobile (the survival table should be biased toward death and notification, not preservation); the one-thread grammar becomes a triage QUEUE of decisions-under-clock; the flawless flow is the escalated bid/trade arriving as a push → opening straight into the Control Card → resolved with one hold-to-fire, never making the operator navigate. The casual gets a calm, near-empty phone that only speaks when it matters (no notification anxiety); the operator gets a ruthless triage feed across 25 leagues. Then be honest in RIPCORD about where the "reactive inbox" model FAILS — the operator who wants to PROACTIVELY interrogate ("how do my running backs look across all my leagues?") gets nothing from an inbox, and a pure-notification phone can become the exact notification-anxiety machine the casual was promised freedom from. Name where reactive-triage is the wrong instrument, and what proactive affordance has to coexist with it.
