...
// value returns the effective value for a parameter — the admin override if present,
// otherwise the shipped default. ok is false for an unknown (key, position).
...
// GetCapTiers returns the Layer-5 cap-scaling boundaries (AD-21), override-aware.
func (s *Store) GetCapTiers() (CapTiers, error) {
	cold, ok := s.value(KeyCapTierColdCeiling, global)
	if !ok {
		return CapTiers{}, fmt.Errorf("params: missing %s", KeyCapTierColdCeiling)
	}
	hot, ok := s.value(KeyCapTierHotFloor, global)
	if !ok {
